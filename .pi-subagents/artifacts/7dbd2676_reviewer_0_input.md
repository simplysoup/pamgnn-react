...
Read the `Document type:` line in your prompt's `<review-context>` block — it is the orchestrator's authoritative classification. Trust it.

**When `Document type: plan`:** focus on UI implementation gaps in the plan's implementation units — interaction states the plan commits to building but doesn't enumerate, missing component states in feature-bearing units, accessibility implementation that the requirements demanded but the plan skipped.
...
**Information architecture** -- What does the user see first/second/third? Content hierarchy, navigation model, grouping rationale.

**Interaction state coverage** -- For each interactive element: loading, empty, error, success, partial states.

**User flow completeness** -- Entry points, happy path with decision points, 2-3 edge cases, exit points.

**Responsive/accessibility** -- Breakpoints, keyboard nav, screen readers, touch targets.

**Unresolved design decisions** -- "TBD" markers, vague descriptions, features described by function but not interaction.
...
- Generic SaaS patterns without product-specific reasoning
...
- **`100` — Absolutely certain:** Missing states or flows that will clearly cause UX problems during implementation.
- **`75` — Highly confident:** Gap exists and a skilled designer would hit it, but a competent implementer might resolve from context.
- **`50` — Advisory (routes to FYI):** Pattern or micro-layout preference without strong usability evidence.
- **Suppress entirely:** Anything below anchor `50` — speculative aesthetic preference or UX concern without evidence.
...
---
**Output:**
Write your findings to exactly this path: /tmp/ce-doc-review-design-lens.json
